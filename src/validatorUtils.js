 ...
 * @Date: 2019-08-22 16:00:49
 * @LastEditors: OBKoro1
 * @LastEditTime: 2019-08-22 17:18:56
 */
/* eslint-disable */
/**
...
const REGEX_RULE = {
  loginip: /^1\d{10}$/,
  // 税号
  duty: /^[A-Z0-9]{15}$|^[A-Z0-9]{17}$|^[A-Z0-9]{18}$|^[A-Z0-9]{20}$/,
  // 匹配email
  ...
  url: /^[httphttps]+:\/\/([A-Za-z0-9\.:]+)+(\/[\w-]+)+\/[\w-]+.html/g,
  filterUrl: /(\/[\w-]+)+\/[\w-]+.html/g,
  noTitle: /^[\u4e00-\u9fa5_a-zA-Z0-9]+$/gi
}
const seotitle = text => {
...
// 税号
const duty = text => {
  return REGEX_RULE.duty.test(text)
}
/**
 * email验证器
...
const loginiP = text => {
  return REGEX_RULE.loginiP.test(text)
}
...
const noTitle = text => {
  return REGEX_RULE.noTitle.test(text)
}
